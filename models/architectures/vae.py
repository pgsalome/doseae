...
class VAE(BaseAutoencoder):
    ...
    def compute_loss(self, outputs, batch, beta: float = 1.0):
        """Compute VAE losses in the format expected by the Trainer."""
        if not isinstance(outputs, dict):
            raise ValueError('Expected dictionary outputs for VAE compute_loss.')
        ...
        if reconstruction is None or mu is None or logvar is None:
            raise ValueError('VAE outputs must contain reconstruction, mu, and logvar tensors.')

        target = None
        for key in ('target', 'dose', 'dose_patches', 'input', 'ct', 'image'):
        ...
        if target is None:
            raise ValueError('Unable to locate target tensor in batch for VAE loss computation.')

        losses = self.get_losses(target, reconstruction, mu, logvar, beta=beta)
        return {
            'reconstruction': losses['recon_loss'],
            'kl': losses['kl_loss'],
