...
class VAE2D(BaseAutoencoder):
    ...
    def compute_loss(self, outputs, batch, beta: float = 1.0):
        """Compute loss dictionary compatible with the Trainer."""
        if not isinstance(outputs, dict):
            raise ValueError('Expected dictionary outputs for VAE2D compute_loss.')
        ...
        if reconstruction is None or mu is None or logvar is None:
            raise ValueError('VAE2D outputs must contain reconstruction, mu, and logvar tensors.')

        target = None
        for key in ('target', 'dose', 'dose_image', 'input', 'ct', 'image'):
        ...
        if target is None:
            raise ValueError('Unable to locate target tensor in batch for VAE2D loss computation.')
