...
PROBLEMATIC_PATIENTS = ["0617535250", "0617445118","0617449451"]
...
def run_lung_segmentation(ct_path, seg_output_dir, file_ext):
    ...
    try:
        ...
        right_lung_img.CopyInformation(combined_mask_image) # <-- ADD THIS LINE
        ...
        left_lung_img.CopyInformation(combined_mask_image) # <-- AND ADD THIS LINE
...
# --- Find this function and replace it with this new version ---

def apply_mask_and_normalize_dose(dose_image, mask_path, output_path):
    """
    Applies a lung mask to a dose image object and normalizes it by the 95th percentile.
    This version is optimized to take an image object instead of a path.
    """
    # The function no longer needs to load the dose_image, it's passed in.
    mask_image = None
    masked_dose_image = None
    ...
    try:
        ...
        del mask_image # Clean up mask immediately
        gc.collect()

        dose_array = sitk.GetArrayViewFromImage(masked_dose_image)

        positive_voxels = dose_array[dose_array > 0]
        if positive_voxels.size == 0:
            logger.warning("No non-zero dose values found in the masked region.")
            return None

        percentile_95 = np.percentile(positive_voxels, 95)

        del dose_array, positive_voxels
        gc.collect()
        ...
        logger.error(f"Failed during masking and normalization: {e}")
        import traceback
        logger.error(traceback.format_exc())
        return None
    finally:
        del mask_image, masked_dose_image, normalized_image
        gc.collect()
...
def extract_and_process_patches(normalized_masked_dose_path, config, subject_id, lung_side):
    ...
    try:
        ...
        # No longer needed, as we read voxels directly
        ...
        # The rest of the function remains the same, but we comment out the old calculation
...
def process_patient(rtdose_dir, output_dir, config):
    ...
    # 1. Find DICOMs and check processability (same as before)
    ct_dicom_dirs = [x for x in glob.glob(join(dirname(dirname(dirname(dirname(rtdose_dir)))), '*/*/*CT-*MP1*/*')) if 'RTSTRUCT' not in x]
    ...
    # 2. Convert DICOMs (same as before)
    ...
    if not os.path.exists(ct_conv_path):
        if not run_dicom_conversion(ct_dicom_dir, convert_to_keyword): return 0
    if not os.path.exists(dose_conv_path):
        if not run_dicom_conversion(rtdose_dir, convert_to_keyword): return 0

    # 3. Resample CT and Dose (same as before)
    ...
    # 4. Segment Lungs (same as before)
    ...
    # --- NEW LOGIC: DETERMINE IPSI/CONTRA SIDE ---
    try:
    ...
    # 5. Process each lung (MODIFIED to use the new labels)
    patient_patches = []
    total_patches = 0
    ...
    for lung_side, mask_path in [("left", left_lung_mask_path), ("right", right_lung_mask_path)]:
        ...
        lung_patches = []
        # Pass the clinical_side to the patch extractor
        ...
        logger.info(f"Extracted {len(lung_patches)} patches from {lung_side} ({clinical_side}) lung.")
        patient_patches.extend(lung_patches)
        total_patches += len(lung_patches)

    # 6. Save patches (same as before)
    if total_patches > 0:
